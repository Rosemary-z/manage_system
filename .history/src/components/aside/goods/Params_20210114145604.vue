...
export default {
  ...
  methods: {
    ...
    handleClose(index, tagData) {
      // 关闭标签触发的函数，删除当前tag
      console.log("当前想要关闭的tag参数索引", index);
      console.log("当前想要关闭的tag参数数据数组", tagData);
      tagData.attr_vals.splice(index, 1);
      this.manyTabData = tagData;
      console.log("删除了当前tag之后的本行参数数据", this.tagData);
    },
    showInput() {
    ...
    handleInputConfirm(rowAttrData) {
      ...
      console.log("增加或删除之后的属性列表", this.manyTabData.attr_vals);
