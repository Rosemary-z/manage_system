<template>
  <div>
    ...
    >
      <el-form :model="addForm" :rules="formRules" ref="addFromRef" label-width="100px">
  ...
  methods: {
    ...
    addParams() {
      this.$refs.addFromRef.validate(async valid => {
        ...
        if (res.meta.status !== 201) {
          return this.$message.error("添加" + this.paramsText + "失败！");
        }
        this.$message.success("添加" + this.paramsText + "成功！");
    ...
    editParams() {
      this.$refs.editFromRef.validate(async valid => {
        ...
        if (res.meta.status !== 200) {
          return this.$message.error("修改" + this.paramsText + "失败！");
        }
        this.$message.success("修改" + this.paramsText + "成功！");
        this.getParamsData();
    ...
    async removeParams(attrId) {
      ...
      if (res.meta.status !== 200) {
        return this.$message.error("删除" + this.paramsText + "失败！");
      }
      this.$message.success("删除" + this.paramsText + "成功！");
      this.getParamsData();
    },
    // 文本框失去焦点,或者按下Enter触发
    ...
    async saveAttrVals(row) {
      ...
      if (res.meta.status !== 200) {
        return this.$message.error("修改" + this.paramsText + "项失败");
      }
      this.$message.success("修改" + this.paramsText + "项成功");
    },
    // 点击按钮显示输入框
    showInput(row) {
